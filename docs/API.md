...
| Param | Type | Default | Notes |
|---|---|---|---|
| description | string | required | e.g. "add a nullable note column to orders" |
| allow_drops | bool | false | same meaning as in `migration.plan` |
| dry_run | bool | true | same meaning as in `migration.plan` |

Returns everything `migration.plan` returns, plus `interpreted_target` and
`description` — so you review the *interpretation*, not just the SQL it produced. A
wrong target that happens to generate valid SQL is the failure mode this surfaces.

**The translation runs on your model, not ours.** This uses MCP sampling: the server
asks the connected client for a completion, so pgops ships no API key, no model config
and no inference bill. Your client stays in control of approving each request.

**Sampled output is never executed.** The pipeline is:

```
description → [your model] → target JSON → _validate_target
            → deterministic differ → lock analysis → dry run → plan
```

The model proposes a *destination*. It never writes SQL, and every statement is still
generated by the same differ, annotated by the same lock analysis and gated by the same
confirmation flow that a hand-written target faces. A hallucinated column shows up as an
`ADD COLUMN` step you can see and reject; it cannot bypass a guardrail, because it never
produces a statement in the first place.

Behaviour worth knowing:
- Requires a client that supports sampling. Without it the call fails with
  `SAMPLING_UNAVAILABLE` and a hint containing the equivalent `migration.plan` call —
  not an apology.
- If the request is ambiguous the model is instructed to say so rather than guess; that
  refusal is relayed as `INVALID_ARGUMENT` naming what was unclear. Planning a migration
  from an invented interpretation is the outcome this avoids.
- The live schema is sent with the request, because the target describes the desired
  *final state* and therefore has to restate columns the table already has.
- The result is a plan, not an application. Applying still requires `migration.apply`
