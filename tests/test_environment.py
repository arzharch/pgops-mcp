...
DEV_DSN = "postgresql://pgops:pgops_dev@localhost:5433/pgops_demo"
...
async def test_topology_identifies_our_database_by_port() -> None:
    """This machine runs several postgres containers at once — ours on host port 5433,
    an unrelated project's on 5434. Matching on "the image is postgres" would
    ...
    topology = await env_topology(_config())
    assert topology["dsn_host_port"] == 5433
    db = topology["database_container"]
    assert db is not None
    assert db["name"] == DEV_CONTAINER
    assert any(p["host_port"] == 5433 for p in db["ports"])
