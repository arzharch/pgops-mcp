...
async def migration_apply(
    ...
    needs_confirmation = any(s.change.destructive for s in plan.steps) or plan.highest_risk == "high"
...
def _confirmation_reason(plan: MigrationPlan) -> str:
    ...
    if destructive:
        losses = "; ".join(
            s.change.data_loss_reason or s.change.sql for s in destructive[:3]
        )
...
async def migration_history(conn_manager: ConnectionManager, limit: int = 20) -> dict[str, Any]:
    ...
    return {
        ...
        "warning": (
            ...
            "half-migrated" if in_flight else None
        ),
    }
